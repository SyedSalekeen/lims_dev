@extends('user.layouts.master')

@section('content')
    <!-- BEGIN: Content-->
    <!-- eCommerce statistic -->
    <div>
        <div class="setting-top">
            <h4>Settings</h4>
            <i class="fas fa-cog"></i>
        </div>
        <div class="doctor-profile">
            <div class="d-flex align-items-center">
                @if ($get_auth_user->profile_image)
                    <div class="doctor-profile-img">
                        <img src="{{asset('uploads/profile/'.$get_auth_user->profile_image)}}"
                            alt="">
                    </div>
                @else
                    <div class="doctor-profile-img">
                        <img src="data:image/png;base64,"
                            alt="">
                    </div>
                @endif
                <h3>Welcome <strong>{{$get_auth_user->first_name.' '.$get_auth_user->last_name}}</strong></h3>
            </div>
            <div class="doctor-logo">
                {{-- <i class="fas fa-clinic-medical"></i> --}}
            </div>
        </div>
        <div class="card-row">
            <div class="row">

                <div class="col-lg-3">
                    <div class="card-patient">
                        <a href="{{route('branch_user.index')}}">

                            {{-- <div class="for-span">
                                <span class="label label-info">{{$employees_details }}</span>
                               </div> --}}
                        <div>
                            <h2>Employees<h2>
                        </div>
                        <div>
                            <i class="fad fa-users"></i>
                        </div>
                    </a>
                    </div>
                </div>
                <div class="col-lg-3">
                    <div class="card-patient">
                        <a href="{{route('branch_patient.index')}}">
                           {{-- <div class="for-span">
                            <span class="label label-info">{{ $Patients_details }} </span>
                           </div> --}}

                        <div>

                            <h2> Patients<h2>
                        </div>
                        <div>
                            <i class="fas fa-procedures"></i>
                        </div>
                        </a>
                    </div>

                </div>

                <div class="col-lg-3">
                    <div class="card-patient">
                        {{-- {{ route('Aggregate_Data.show') }} --}}
                        <a href="">
                            {{-- <div class="for-span">
                                <span>- -   </span>
                               </div> --}}
                        <div>
                            <h2>Machine Down Time<h2>
                        </div>
                        <div>
                            {{-- <i class="fad fa-database"></i> --}}
                            <i class="fas fa-history"></i>
                        </div>
                        </a>
                    </div>
                </div>

                <div class="col-lg-3">
                    <div class="card-patient">
                        <a href="{{ route('branch_expensive_report.index') }}">
                            {{-- <div class="for-span">
                                <span>{{ $Expenses_details }} </span>
                               </div> --}}
                        <div>
                            <h2>Expense<h2>
                        </div>
                        <div>
                            <i class="fas fa-money-check-edit-alt"></i>
                        </div>
                        </a>
                    </div>
                </div>

            </div>
        </div>

            {{-- ================= ROW TWO =============== --}}
        <div class="card-row">
            <div class="row">
                <div class="col-lg-3">
                    <div class="card-patient">
                        {{-- {{ route('User_report.show') }} --}}
                        <a href="branch_patient_report.index">
                            {{-- <div class="for-span">
                                <span>{{ $Reports_details }} </span>
                               </div> --}}
                        <div>
                            <h2>Medical Reports<h2>
                        </div>
                        <div>
                            <i class="fad fa-files-medical"></i>
                        </div>
                    </a>
                    </div>
                </div>
                <div class="col-lg-3">
                    <div class="card-patient">
                        {{-- {{ route('employees.show') }} --}}
                        <a href="{{ route('branch_profit_report.index') }}">
                            {{-- <div class="for-span">
                                <span>{{ $NetProfit_details }} </span>
                               </div> --}}
                        <div>
                            <h2></span> Net Profit<h2>
                        </div>
                        <div>
                            <i class="fad fa-sack-dollar"></i>
                        </div>
                        </a>
                    </div>
                </div>
                </div>
            </div>


        </div>
        <br>
    <!-- END: Content-->
@endsection
