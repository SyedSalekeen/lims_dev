...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            ...
            @if (Session::get('profit_report_store'))
                <?php
                session()->forget('profit_report_store');
                ?>
                <div class="alert my-2 alert-success">
                    <p>{{ 'Profit Report has been added' }}</p>
                </div>
            @endif
            ...
            @endif
            @if (Session::get('one_field_required'))
                <?php
                session()->forget('one_field_required');
                ?>
                <div class="alert my-2 alert-danger">
                    <p>{{ 'One field is required' }}</p>
                </div>
            @endif
            @if (Session::get('profit_report_delete'))
                <?php
                session()->forget('profit_report_delete');
                ?>
                <div class="alert my-2 alert-success">
                    <p>{{ 'Profit Report has been deleted' }}</p>
                </div>
            @endif
            @if (Session::get('vendor_profit_one_fileld_required'))
                <?php
                session()->forget('vendor_profit_one_fileld_required');
                ?>
                <div class="alert my-2 alert-danger">
            ...
            {!! Form::open(['route' => 'profit_report.filterReports', 'method' => 'POST']) !!}
            <div class="form form-horizontal mb-5">
                <div class="form-body">
                    <h4 class="form-section"><i class="ft-user"></i>Filter Profit Report</h4>
            ...
            <div class="card mt-5">
                ...
                <div class="col-xs-12 col-sm-12 col-md-12">
                    {{-- {{ route('branch.index') }} --}}
                    <a href="{{ route('export_profit') }}"
                        class="btn back-button-color float-left box-shadow-1 mt-1 mb-1 mr-1"><i class="ft-arrow-left"></i>
                        Export to Excel </a>
                </div>


                <div class="card-header">
                    <h4 class="card-title">Profit Reports</h4>
                    ...
                    <div class="heading-elements">
                        <ul class="list-inline mb-0">
                            ...
                            <li>
                                <a href="{{ route('profit_report.create') }}"><button class="btn create-new-button"><i
                ...
                <div class="card-content collapse show">
                    ...
                    {{-- <div class="table-responsive">
                        <table class="table table-striped table-sm">
                            ...
                            <tbody>
                                @foreach ($get_profit_reports as $value)
                                    <tr role="row">
                                        <td>{{ $loop->index + 1 }}</td>
                                        <td>{{ $value->profit_name }}</td>
                                        <td>{{ date('F d, Y', strtotime($value->created_at)) }}</td>
                                        <td>{{ $value->profit_amount }}</td>
                                        <td>{{ $value->profit_description }}</td>
                                        <td class="d-flex border-0">
                                            <a class="btn btn-primary box-shadow-1 btn-sm mr-1"
                                                href="{{ route('profit_report.edit', $value->id) }}"><i
                                                    class="ft-edit"></i></a>
                                            {!! Form::open(['method' => 'DELETE', 'route' => ['profit_report.destroy', $value->id], 'style' => 'display:inline']) !!}
